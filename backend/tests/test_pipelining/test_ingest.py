...
def test_ingest_with_bad_ae_title(mock_images_folder):
    ...
    with pytest.raises(NotImplementedError):
        DicomIngestController.ingest_folder (
            mock_images_folder, "test", "test", "0", "BAD_AET"
            )
...
def test_ingest_folder_to_bad_pipeline(mock_images_folder):
    # TODO update test when edge case is implemented
    with pytest.raises(NotImplementedError):
        assert not DicomIngestController.ingest_folder (
            mock_images_folder, "test", "test", "0", config.PIPELINE_AE_PREFIX + "non-existant-ae"
            )
...
def test_ingest_folder_to_valid_pipeline(mock_images_folder, db):
    ...
    assert DicomIngestController.ingest_folder (
        mock_images_folder, "test", "test", "0", config.PIPELINE_AE_PREFIX + ae_title
        )
