<template>
  <v-row class="pipeline-creator" no-gutters>
    ...
    <v-col cols="12">
      <v-row class="ma-2" style="position: absolute; z-index: 1;" no-gutters align="center">
        ...
        <v-btn @click="containerList = !containerList" class="ml-2" large icon>
          <v-icon large color="#373740" v-text="containerList ? 'mdi-minus' : 'mdi-plus'"/>
        </v-btn>
      </v-row>
      ...
      <v-navigation-drawer v-model="containerList" absolute right>
        <template v-slot:prepend>
          <v-list-item two-line>
            <v-list-item-content>
              ...
              <v-list-item-subtitle>A list of all the containers</v-list-item-subtitle>
            </v-list-item-content>
          </v-list-item>
        </template>
        <v-divider/>
        ...
        <v-hover v-for="container in containers" :key="container.id" v-slot:default="{ hover }">
          <v-card class="ma-2 title" :color="hover ? 'orange': ''">
            <v-card-title v-text="container.title"/>
            <v-card-text>
              Lorem ipsum dolor sit amet, consectetur adipiscing elitonec.
            </v-card-text>
            <v-card-actions>
              <v-spacer/>
