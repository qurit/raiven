...
export const validateDomain = v => /(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z0-9][a-z0-9-]{0,61}[a-z0-9]/.test(v)
export const validateIP = v => /\b(?:(?:2(?:[0-4][0-9]|5[0-5])|[0-1]?[0-9]?[0-9])\.){3}(?:(?:2([0-4][0-9]|5[0-5])|[0-1]?[0-9]?[0-9]))\b/.test(v)
export const validatePort = v => /^([1-9]|[1-5]?[0-9]{2,4}|6[1-4][0-9]{3}|65[1-4][0-9]{2}|655[1-2][0-9]|6553[1-5])$/.test(v) || 'Field is not a valid port'
...
export function validateHostAddress(v) {
  if (!!v) {
    if (!validateDomain(v) && !validateIP(v)) return 'Field is not a valid host address'
    return true
